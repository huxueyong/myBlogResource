...
function loadNetworkArticlesAllByPage(nacsID,num) {//nacsID此时的值为0，代表不区分文章类别
    ...
    if (loadNetworkArticlesByPageArr[nacsID+"-"+num]) {
        console.log("缓存被调用了！")
        dataList = loadNetworkArticlesByPageArr[nacsID+"-"+num].data;
        ...
        var recordCount = loadNetworkArticlesByPageArr[nacsID+"-"+num].recordCount;
        updatePageNum(num, pageCount, nacsID,nasourceID,recordCount);
...
function updatePageNum(num, pageCount, nacsID,nasourceID,recordCount) {
    var pageHtml = "";
    if (num == 1) {
        if (pageCount <= 5) {
            ...
            pageHtml += `</ol><p class="pageCount">共 <span>${pageCount}</span> 页，总计 <span>${recordCount}</span> 条记录</p>`;
        } else {//pageCount>5
            pageHtml += `<ol class="pageList" data-naCategorySecond-id="${nacsID}" data-naSource-id="${nasourceID}" data-record-count="${pageCount}"><li class="active"><a href="1">1</a></li><li><a href="2">2</a></li><li><a href="3">3</a></li><li><a href="4">4</a></li><li><a href="5">5</a></li></ol><p class="pageCount">共 <span>${pageCount}</span> 页，总计 <span>${recordCount}</span> 条记录</p>`;
        }
    } else if (num == 2) {
        if (pageCount <= 5) {
            ...
            pageHtml += `<ol class="pageList" data-naCategorySecond-id="${nacsID}" data-naSource-id="${nasourceID}" data-record-count="${pageCount}"><li><a href="1">1</a></li><li class="active"><a href="2">2</a></li><li><a href="3">3</a></li><li><a href="4">4</a></li><li><a href="5">5</a></li></ol><p class="pageCount">共 <span>${pageCount}</span> 页，总计 <span>${recordCount}</span> 条记录</p>`;
        }
    } else if (num >= 3) {
        ...
        if (pageCount - num < 2) {
            ...
            pageHtml += `<ol class="pageList" data-naCategorySecond-id="${nacsID}"  data-naSource-id="${nasourceID}" data-record-count="${pageCount}"><li><a href="${num - 2}">${num - 2}</a></li><li><a href="${num - 1}">${num - 1}</a></li><li class="active"><a href="${num}">${num}</a></li><li><a href="${num + 1}">${num + 1}</a></li><li><a href="${num + 2}">${num + 2}</a></li></ol><p class="pageCount">共 <span>${pageCount}</span> 页，总计 <span>${recordCount}</span> 条记录</p>`;
        }
    }
...
function updateNotesList(dataList) {
    ...
    $.each(dataList, function (i, v) {
        ...
        nabodyHtml += `<tr>
			                ...
			                <td class="naTitle"><a href="http://www.huxueyong.com/network_content.html?naID=${v.networkArticle_id}" target="_blank" data-networkArticle-id="${v.networkArticle_id}">${v.networkArticle_title}</a></td>
...
function loadNetworkArticlesBynacsIDNum(nacsID,num) {
    ...
    if (loadNetworkArticlesByPageArr[nacsID + "-" + num]) {//从缓存中获取数据
        ...
        // //3.更新页码条信息
        updatePageNum(num, pageCount, nacsID,0,recordCount);
