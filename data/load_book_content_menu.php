...
// `bookCon_id`, `bookCon_title`, `bookCon_content`, `bookCon_menuName`, `bookCon_menulevel`, `bookCon_menubelong`, `bookCon_order`, `bookCon_bookListID`

// SELECT `bookCon_bookListID`,GROUP_CONCAT(`bookCon_id` ORDER BY `bookCon_order` ASC),GROUP_CONCAT(`bookCon_title` ORDER BY `bookCon_order` ASC) FROM `book_content` WHERE `bookCon_menulevel`=2 AND `bookCon_menubelong`=2 GROUP BY `bookCon_menubelong`;

// SELECT GROUP_CONCAT(`bookCon_id` ORDER BY `bookCon_order` ASC),GROUP_CONCAT(`bookCon_title` ORDER BY `bookCon_order` ASC) FROM `book_content` WHERE `bookCon_menulevel`=2 AND `bookCon_menubelong`=1 GROUP BY `bookCon_menubelong`;

// SELECT `bookCon_menubelong`,`bookCon_title`,GROUP_CONCAT(`bookCon_id` ORDER BY `bookCon_order` ASC),GROUP_CONCAT(`bookCon_title` ORDER BY `bookCon_order` ASC) FROM `book_content` WHERE `bookCon_menulevel`=2 AND `bookCon_menubelong`=1 GROUP BY `bookCon_menubelong` 