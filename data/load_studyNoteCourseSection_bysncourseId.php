...
if($outputlength!==0){
		for($i=0;$i<$outputlength;$i++){
				...
				$sql="SELECT `sncc_id`,`sncc_name`,`sncourse_id`,`sncourse_isVip` FROM `study_note_course_content`,`study_note_course` WHERE `sncourse_id`=`sncc_sncourseId` AND `sncc_sncsectionId`=$sncsectionId ORDER BY `sncc_order` ASC";
				$result = mysqli_query($conn,$sql);
		...
		$sql="SELECT `sncc_id`,`sncc_name`,`sncourse_id`,`sncourse_isVip` FROM `study_note_course_content`,`study_note_course` WHERE `sncourse_id`=`sncc_sncourseId` AND `sncc_sncourseId`=$sncourseId ORDER BY `sncc_order` ASC";
